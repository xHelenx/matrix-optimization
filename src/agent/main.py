...
from globalConstants import  COMMAND_EVALUATION_DONE, COMMAND_SETUP_DONE, COMMAND_TRAINING_DONE, EVENT_INFO,  EXTENSION_LOG, LOGFILE,  EVENT_CONFIG,EVENT_REWARD,EVENT_COMMAND, \
...
if __name__ ==  "__main__":
    ...
    while not env.dataEx.received_info: #wait to receive info
        sleep(SLEEP_TIME)
        print(os.path.exists(FILE_PATH + EVENT_INFO + EXTENSION_XML))
        if os.path.exists(FILE_PATH + EVENT_INFO + EXTENSION_XML):
            debug_print("info.xml exists", DEBUG_FILECREATION)
            env.dataEx.read_file(EVENT_INFO)
        
    env.dataEx.received_info = False
    # Evaluate 
    num_updates = 0
    ...
    env.dataEx.write_command(commandtype=COMMAND_EVALUATION_DONE) 
